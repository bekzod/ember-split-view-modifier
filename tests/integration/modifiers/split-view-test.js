...
module("Integration | Modifier | split-view", function (hooks) {
  ...
  test("No gutter exists when the number of children is not greater than 1", async function (assert) {
    await render(hbs`
      <div {{split-view}} class='container'>
        <div class='child' />
      </div>
  ...
  test("Passing the direction property with the value vertical splits up its children vertically", async function (assert) {
  ...
  test("Split view modifier rerenders when arguments update", async function (assert) {
    set(this, "direction", "vertical");
  ...
  test("Passing in the sizes parameter splits up its children by their respective amount", async function (assert) {
